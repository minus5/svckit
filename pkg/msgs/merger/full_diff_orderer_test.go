...
func full(no int) *msg {
	b := &msgs.Backend{Type: "d_123/full", No: no, Body: []byte(`{}`)}
	return &msg{typ: "d_123/full", isFull: true, no: no, backend: b}
}

func diff(no int) *msg {
	b := &msgs.Backend{Type: "d_123/diff:", No: no, Body: []byte(`{}`)}
	return &msg{typ: "d_123/diff", isDiff: true, no: no, backend: b}
}
