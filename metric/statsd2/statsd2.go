...
// Examples:
//   - Dial()
//   - Dial(statsd2.MetricPrefix("my_app"))
//   - Dial(statsd2.MetricPrefix("my_app"),	statsd2.StatsDAddr( "127.0.0.1:8125"))
func Dial(opts ...Option) error {
	...
	woPrefix := newStatsd2("", currentClient)
...
func newStatsd2(prefix string, client client) *Statsd2 {
	return &Statsd2{
		client:   client,
		prefix:   prefix,
		prefixes: make(map[string]*Statsd2),
	}
}

// Counter increments counter name for sum(values).
// If called witohout values will increment for 1.
func (i *Statsd2) Counter(name string, values ...int) {
	value := 1
	if len(values) > 0 {
		value = 0
...
func (i *Statsd2) Time(name string, duration int) {
	i.client.Timing(i.handlePrefix(name), int64(duration))
	usageSent.Timer()
}

func (i *Statsd2) handlePrefix(name string) string {
	if "" == i.prefix {
		return name
	}
...
func (i *Statsd2) WithPrefix(prefix string) metric.Metric {
	i.mapLock.Lock()
	...
	i.prefixes[prefix] = newStatsd2(mPrefix, i.client)
...
func (i *Statsd2) AppendSuffix(suffix string) metric.Metric {
